...
upload_package() {
  ...
  while true; do
    ...
    if [ "${UPLOAD_STATUS}" = "FAILED" ]; then
      echo "El upload no se completó correctamente, el status es ${UPLOAD_STATUS}"
	    UPLOAD_META=$(echo "${UPLOAD_RESULT}" | jq -r '.upload.metadata')
	    echo "El error del mensaje es ${UPLOAD_META}"
      echo "No se puede continuar con los tests..."
      exit 1
    elif [ "${UPLOAD_STATUS}" != "SUCCEEDED" ]; then
      echo "Upload de ${PACKAGE_PATH} aún no finaliza, el status actual es ${UPLOAD_STATUS}"
      echo "Esperando hasta que el upload termine..."
      sleep 5
    else
      echo "Uploading ${PACKAGE_PATH} está completado con el status ${UPLOAD_STATUS}"
      break
    fi
  done
}
...
SCHEDULED_TEST_RUN_INFO=$(aws devicefarm schedule-run \
  ...
  --test type="${AWS_TEST_TYPE}",testPackageArn="${TEST_UPLOAD_ARN}",testSpecArn="${AWS_TEST_SPEC_ARN}")
